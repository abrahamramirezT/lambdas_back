...
def lambda_handler(event, context):
    ...
    try:
        ...
        if not all([nombre, apellido_paterno, apellido_materno, user, password is not None]):
            raise KeyError('Faltan parámetros requeridos en la solicitud')


        # Insertar datos en la base de datos
        sql = """
        INSERT INTO users_incidencias (nombre, apellido_paterno, apellido_materno, user, password)
        VALUES (%s, %s, %s, %s, %s)
        """
        cursor.execute(sql, (nombre, apellido_paterno, apellido_materno, user, password))
        connection.commit()
