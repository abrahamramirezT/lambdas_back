  ...
  <div>
    ...
    <form @submit.prevent="updateItem">
      ...
      <select id="estatus" v-model="estatus" required>
        <option v-for="(text, value) in estatusOptions" :key="value" :value="value">
          {{ text }}
        </option>
      </select>
...
export default {
    ...
    async fetchItem(reporte_id) {
      try {
        ...
        if (response.data) {
          ...
          this.reporte_id = item.reporte_id || '';
          this.titulo = item.titulo || '';
          this.fecha = item.fecha || '';
          this.descripcion = item.descripcion || '';
          
          // Asegúrate de que el valor de estatus sea un número entero
  ...
  console.log('ID del elemento:', this.reporte_id); // Verifica que el ID esté correcto
