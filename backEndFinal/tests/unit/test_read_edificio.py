...
from read_edificio_all.app import lambda_handler, get_secret
import os
import json

class TestReadEdificioLambda(unittest.TestCase):

    @patch.dict(os.environ, {
        'MY_SECRET_NAME': 'test_secret',
        'MY_AWS_REGION': 'us-west-2'
    })
    @patch('read_edificio_all.app.boto3.session.Session.client')
    ...
    def test_lambda_handler_success(self, mock_connect, mock_boto_client):
        ...
        mock_cursor.fetchall.return_value = [(1, 'Edificio 1'), (2, 'Edificio 2')]
        ...
        self.assertIn('Edificio 1', response['body'])
    ...
    @patch('read_edificio_all.app.boto3.session.Session.client')
    @patch('mysql.connector.connect')
    def test_lambda_handler_no_buildings_found(self, mock_connect, mock_boto_client):
        # Simulación de la respuesta de Secrets Manager
        mock_secrets_client = MagicMock()
        mock_secrets_client.get_secret_value.return_value = {
            'SecretString': json.dumps({
