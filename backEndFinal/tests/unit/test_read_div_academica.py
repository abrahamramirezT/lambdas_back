...
from read_div_academica_all.app import lambda_handler, get_secret
import os
import json

class TestReadDivAcademicaLambda(unittest.TestCase):

    @patch.dict(os.environ, {
        'MY_SECRET_NAME': 'test_secret',
        'MY_AWS_REGION': 'us-west-2'
    })
    @patch('read_div_academica_all.app.boto3.session.Session.client')
    ...
    def test_lambda_handler_success(self, mock_connect, mock_boto_client):
        ...
        mock_cursor.fetchall.return_value = [(1, 'División Académica 1'), (2, 'División Académica 2')]
        ...
        self.assertIn('División Académica 1', response['body'])
    ...
    @patch('read_div_academica_all.app.boto3.session.Session.client')
    @patch('mysql.connector.connect')
    def test_lambda_handler_no_div_academica_found(self, mock_connect, mock_boto_client):
        # Simulación de la respuesta de Secrets Manager
        mock_secrets_client = MagicMock()
        mock_secrets_client.get_secret_value.return_value = {
            'SecretString': json.dumps({
