...
from read_grado_all.app import lambda_handler, get_secret
import os
import json

class TestReadGradoLambda(unittest.TestCase):

    @patch.dict(os.environ, {
        'MY_SECRET_NAME': 'test_secret',
        'MY_AWS_REGION': 'us-west-2'
    })
    @patch('read_grado_all.app.boto3.session.Session.client')
    ...
    def test_lambda_handler_success(self, mock_connect, mock_boto_client):
        ...
        mock_cursor.fetchall.return_value = [(1, 'Grado 1'), (2, 'Grado 2')]
        ...
        self.assertIn('Grado 1', response['body'])
