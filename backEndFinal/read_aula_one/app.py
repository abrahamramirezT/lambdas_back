...
def lambda_handler(event, __):
    ...
    try:
        ...
        aula_id = event['pathParameters']['aula_id']
        if not aula_id:
            return {
                'statusCode': 400,
                'headers': headers,
        ...
        sql = "SELECT * FROM aulas WHERE aula_id = %s"
        ...
        if aula:
            try:
                ...
                return {
                    'statusCode': 200,
                    'headers': headers,
                    'body': json.dumps(aula_dict)
                }
            except Exception as e:
                logger.error(f"Error al procesar el reporte {aula}: {str(e)}")
                return {
                    'statusCode': 500,
                    'headers': headers,
            ...
            return {
                'statusCode': 404,
                'headers': headers,
                'body': json.dumps({'error': f"No se encontró el reporte con ID {aula_id}"})
            }
