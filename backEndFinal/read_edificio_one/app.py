...
def lambda_handler(event, __):
    ...
    try:
        ...
        edificio_id = event['pathParameters']['edificio_id']
        if not edificio_id:
            return {
                'statusCode': 400,
                'headers': headers,
        ...
        sql = "SELECT * FROM edificios WHERE edificio_id = %s"
        cursor.execute(sql, (edificio_id,))
        ...
        if edificio:
            ...
            return {
                'statusCode': 404,
                'headers': headers,
                'body': json.dumps({'error': f"No se encontró el reporte con ID {edificio_id}"})
            }
